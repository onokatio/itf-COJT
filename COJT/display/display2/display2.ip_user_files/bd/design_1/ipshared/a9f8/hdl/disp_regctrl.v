...
reg [2:0] DSP_VSYNC_X_fix;
reg DSP_VSYNC_negedge;

always @( posedge ACLK ) begin
    if (ARST) begin
        DSP_VSYNC_X_fix[2:0] <= 3'b0;
        DSP_VSYNC_negedge <= 1'b0;
    end else begin
        DSP_VSYNC_X_fix[2:0] <= {DSP_VSYNC_X_fix[1],DSP_VSYNC_X_fix[0],DSP_VSYNC_X};
        DSP_VSYNC_negedge <= DSP_VSYNC_X_fix[2] & ~DSP_VSYNC_X_fix[1];
    end
end
...
wire    DISPADDR_r = (read_reg && RDADDR[11:2]==10'h000);
wire    DISPCTRL_r = (read_reg && RDADDR[11:2]==10'h001);
wire    DISPINT_r =  (read_reg && RDADDR[11:2]==10'h002);
wire    DISPFIFO_r = (read_reg && RDADDR[11:2]==10'h003);
...
always @( posedge ACLK ) begin
    if ( ARST )
        VBLANK = 1'b0;
    else if ( DSP_VSYNC_negedge == 1'b1)
        VBLANK <= 1'b1;
...
always @( posedge ACLK ) begin
    if ( ARST )
        DSP_IRQ <= 1'b0;
    else if ( DSP_VSYNC_negedge == 1'b1 && INTENBL == 1'b1 )
        DSP_IRQ <= 1'b1;
...
always @(posedge ACLK) begin
    if (ARST) begin
        ...
        RDATA[31:0] <= {30'b0, VBLANK, DISPON};
    end else if (DISPINT_r) begin
        RDATA[31:0] <= {31'b0, INTENBL};
    end else if (DISPFIFO_r) begin
        RDATA[31:0] <= {30'b0, FIFOOVER, FIFOUNDER};
    end
end
