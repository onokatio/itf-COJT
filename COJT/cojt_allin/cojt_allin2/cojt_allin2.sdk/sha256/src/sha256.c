...
Word Usigma0_v2(Word x){ return ((x >> 2) | (x << (32 - 2))) ^ ((x >> 13) | (x << (32 - 13))) ^ ((x >> 22) | (x << (32 - 22)));}
Word Usigma1_v2(Word x){ return ((x >> 6) | (x << (32 - 6))) ^ ((x >> 11) | (x << (32 - 11))) ^ ((x >> 25) | (x << (32 - 25)));}
Word Lsigma0_v2(Word x){ return ((x >> 7) | (x << (32 - 7))) ^ ((x >> 18) | (x << (32 - 18))) ^ (x >> 3);}
Word Lsigma1_v2(Word x){ return ((x >> 17) | (x << (32 - 17))) ^ ((x >> 19) | (x << (32 - 19))) ^ (x >> 10);}
...
void schedule(Word block[16],Word result[64]){
    ...
    for(int t=16;t<64;t++){
        result[t] = Lsigma1_v2(result[t-2]) + result[t-7] + Lsigma0_v2(result[t-15]) + result[t-16];
    }
}
...
ABCDEFG ABCDEFG_to_ABCDEFG_vHardware(ABCDEFG src, Word W[64], int count){
    Word K[64] = {
    ...
    //xil_printf("\n");
    xil_printf("orig A = %08x\n",src.A);
    xil_printf("writed A = %08x\n",DRAW_in_A);
    xil_printf("read A = %08x\n",DRAW_out_A);
    ...
    if(count == 0 || count == 10){
    	xil_printf("init[1] = %04x,%04x,%04x,%04x,",dst.A,dst.B,dst.C,dst.D);
    	xil_printf("%04x,%04x,%04x,%04x\n",dst.E,dst.F,dst.G,dst.H);
    }
    return dst;
}
...
ABCDEFG ABCDEFG_to_ABCDEFG_v2(ABCDEFG src, Word W[64], int count){
    ...
    Word T1 = W[count] + K[count] + src.H + Ch(src.E,src.F,src.G) + Usigma1_v2(src.E);
    Word T2 = Maj(src.A,src.B,src.C) + Usigma0_v2(src.A);
    dst.H = src.G;
    dst.G = src.F;
    dst.F = src.E;
    dst.E = src.D + T1;
    ...
    if(count == 0){
    	xil_printf("init[1] = %04x,%04x,%04x,%04x,",dst.A,dst.B,dst.C,dst.D);
    	xil_printf("%04x,%04x,%04x,%04x",dst.E,dst.F,dst.G,dst.H);
    	xil_printf(" T1,T2=%04x,%04x\n",T1,T2);
    }
    return dst;
}
...
void sha256_v3(Word block[16]){
    ...
    for(int i=0;i<64;i++){
        ...
        init = ABCDEFG_to_ABCDEFG_vHardware(init, W, i);
    }
